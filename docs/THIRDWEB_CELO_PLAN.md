...
- `Recommended as a supported path, but not as the only path until validated`
...
- MetaMask works on at least one reliable sponsored path
...
2. `Celo feeCurrency` direct wallet tx
3. normal `CELO` direct wallet tx
...
3. MetaMask external-wallet validation on `celoSepolia`
4. direct `feeCurrency` fallback where possible
5. direct `CELO` fallback for everything else
