...
You need **seven operational wallets** across three security tiers. Never reuse keys across roles.
...
Treasury routing follows governance automatically on non-local deploys, so no separate treasury address env var is
required.
...
This will:
1. Deploy `TimelockController` (2-day delay) + `CuryoGovernor` as the governance authority
...
4. Initialize timelock-owned proxy admins, keep treasury-specific roles on governance, and deploy the stateless `SubmissionCanonicalizer` helper used internally by `ContentRegistry` URL canonicalization
5. Wire cross-contract references
6. Seed 12 content categories with their approved domains and subcategories
7. Mint token allocations: 51,899,900→HumanFaucet, 34M→ParticipationPool, 4M→ConsensusReserve, 100K→KeeperRewardPool, 10M→Governance Treasury
8. **Renounce deployer's temporary admin and treasury roles** — governance, config, and treasury routing transfer fully to TimelockController
9. **Automatically verify** that governance owns the expected upgrade/config/treasury roles and proxy admins, and the deployer retained none
...
- [ ] Treasury-specific roles on `ContentRegistry` / `ProtocolConfig` belong to `TimelockController`
...
refill must include **both** an ERC-20 approval and the funding call in the **same governance execution batch**.
...
4. Create and pass a governance proposal whose TimelockController execution batch makes these calls, in order:
...
5. Queue the proposal, wait out the timelock delay, and execute the batch once it is ready.
6. Verify the refill:
...
If both pools are low, batch all three calls into a single governance execution:
...
This keeps the approval scope minimal and avoids leaving a large standing allowance from the governance treasury to
`RoundVotingEngine`.
