# Curyo 2.0 Question Reward Pools - Initial Research
...
The proposed Curyo 2.0 design would let users create binary questions that can be answered with thumbs up or thumbs down, attach links or images as evidence, and fund those questions with stablecoin-funded Question Reward Pools. The rating flow would remain similar to Curyo's current per-content voting model, but launch-time Question Reward Pools should pay valid revealed participation only from question-scoped funds. cREP should remain the conviction layer where voters risk losing stake when their side loses the settled round.

The direction is promising because it turns Curyo from a general content-rating protocol into a demand-driven human judgment network. The main caution is that "any question, any link, any reward pool" creates legal, moderation, and incentive risks. A safer first version would guide question framing, pay for valid participation and review work, and avoid event-market or wager-like product patterns.

Additional research strengthens the core recommendation: Curyo 2.0 should launch as funded review questions, not as a user-created prediction market. The product should pay verified humans to share bounded judgment under explicit context. It should not let reviewers buy tradable yes/no positions, stake stablecoins to enter, or receive stablecoin payouts for being on the winning side.

## Current Curyo Fit

Curyo already has primitives that map well onto funded human judgment:
...
Question Reward Pools let users ask for a specific judgment instead of hoping a general rating feed produces the signal they need. Examples:
...
A reward pool is a direct signal that someone values an answer. That gives the protocol a natural way to rank review queues, notify qualified voters, and prioritize scarce human attention.

### Stablecoin Rewards Are Easy to Understand

cREP is useful for internal reputation and staking, but stablecoin-funded reward pools are easier for reviewers to price. "Earn USDC for reviewing this question" is a clearer labor-market proposition than "earn protocol reputation emissions."
...
Protocol and frontend fees can be taken from Question Reward Pool funding as payment for routing, moderation, settlement, indexing, and reputation infrastructure. That may be easier to justify than extracting value from ordinary rating activity.
...
- Reward pool size should affect dispute liveness, required quorum, and security parameters.

UMA's bond guidance is especially relevant: higher value at stake should generally mean higher bonds or longer challenge windows, but bonds that are too high can deter honest challengers. Curyo should avoid one static configuration for every reward pool size.
...
MACI exists because public blockchain voting makes bribery easy: voters can prove how they voted. Curyo's tlock commit-reveal model already helps against early herding, but it is not full receipt-free voting. For high-value reward pools, Curyo should consider stronger privacy or receipt-freeness later.

### Gitcoin

Gitcoin's quadratic funding research generalizes to reward-pool questions: when rewards depend on many unique humans, fake or controlled identities become economically valuable. One Voter ID per human is important, but valuable reward pools still need layered defenses such as pattern detection, source-of-funds clustering, community flagging, and category-specific reputation.
...
- Do not launch future-event reward pool categories that look like event contracts, prediction markets, or objective event-resolution products without a separate legal workstream.

### Stablecoin Compliance and Issuer Controls

Stablecoin flows introduce sanctions, blocked-address, prohibited-activity, tax, and issuer-policy considerations. As of this research pass, the U.S. GENIUS Act has created a federal payment-stablecoin framework, and EU MiCA rules regulate asset-referenced tokens and e-money tokens. Even if Curyo is not issuing stablecoins, escrowing and distributing stablecoin-funded reward pools can still require legal review.

Stablecoin issuers may also monitor or block prohibited transactions, including certain gambling or unlawful activity categories. That makes arbitrary reward pools riskier than cREP-only rewards.

Stablecoin-funded reward pool claims may also create tax reporting expectations for users. IRS guidance treats digital assets, including stablecoins, as reportable digital assets and explicitly references rewards, awards, and payment for services. Product copy should assume reward pool recipients may need records of claim amounts, dates, token symbols, and fair market values.

Self.xyz can mitigate part of this risk. Self's disclosure and verification flow supports `excludedCountries` rules and OFAC checks, and the IdentityVerificationHub applies geographic and sanctions requirements from the stored verification configuration. Curyo already uses Self.xyz for Voter ID issuance, with frontend OFAC checking enabled in `packages/nextjs/lib/governance/selfVerificationApp.ts` and on-chain deployment config enabling all three Self OFAC modes in `packages/foundry/script/DeployCuryo.s.sol`. However, the current Curyo config leaves `excludedCountries` empty and `forbiddenCountriesEnabled` false, so sanctioned-country exclusion would need to be explicitly enabled for reward pool participation.

Recommended reward pool-specific approach:

- Require a Voter ID or fresh reward pool eligibility proof minted under a Self configuration that enables OFAC checks and excludes sanctioned or unsupported jurisdictions.
- Version the eligibility config so old Voter IDs minted before the reward pool restrictions can be re-checked before stablecoin funding or claiming.
...
- Apply the gate to reward pool funding, reward pool claiming, and possibly reward pool voting, not only to initial faucet claims.
...
Templates should reduce mismatched expectations, not force every question into objective fact-checking. Reward Pools should require a prompt frame, category, context, and invalid/cannot-resolve criteria, but subjective opinion/rating templates should be first-class.

### Collusion and Bribery

Money increases the payoff from coordination. Curyo's Voter ID system reduces classic sybil attacks, but reward pools can still incentivize:
...
Larger reward pools should require higher quorum, longer windows, more diverse voter participation, and stronger anomaly monitoring.

### Creator Conflicts

The question creator may have a direct interest in the answer. At minimum:

- The creator should not vote on their own reward pool.
- Creator-associated wallets should be excluded where detectable.
- Question metadata should disclose the creator and reward pool source.
- High-value reward pools should have appeal or challenge paths.
...
Curyo already has URL-safety code for SSRF-style risk. Funded questions would need product-level content policy, reporting, takedowns, and evidence retention rules.
...
### Reward Pool Size Can Overwhelm cREP Incentives

If the stablecoin-funded reward pool is much larger than the cREP at risk, voters may optimize for reward capture instead of reputation or rating accuracy. The launch design should avoid hard caps, but larger reward pools should scale with quorum, settled-round requirements, monitoring, and reviewer quality.
...
### 5. Tranche Large Reward Pools

Large reward pools should be released across multiple rounds or confidence milestones. This reduces the incentive to capture a thin first round with a small coordinated group.

### 6. Scale Safety With Reward Pool Size

Higher-value reward pools should automatically require:
...
A funded question likely needs states beyond current content voting:
...
- Should every reward pool require a cREP stake from the question creator?
- Should every reward pool require a separate creator bond that can be slashed independently of the voter reward pool?
- Should voters stake cREP in addition to earning stablecoins?
- Should stablecoin participation rewards be split equally among all valid revealers, pro-rata by capped cREP stake, or through a fixed per-voter reward with unused reward pool refunded or rolled forward?
- Should creators be able to add reward pool after a question is open?
- Should third parties be able to top up existing question-specific reward pools?
- Should unused question reward pool stay attached to later rounds, expire back to funders, or roll into an appeal/challenge tranche?
- Should a question support multiple evidence links or only one canonical link?
- Should high-stakes reward pools require a moderator or curator precheck?
...
- Should reward pool funding and claiming require a fresh Self.xyz proof under a reward pool-specific OFAC and excluded-country configuration?
...
- Should delegated wallets be allowed to fund or claim stablecoin-funded reward pools, or should claims require the direct Voter ID holder?
- Should Curyo add category-specific voter reputation before routing reward pools?
- Should there be an appeal court, expert panel, or governance escalation path?
- How should Curyo handle takedowns when reward pool funds are already escrowed?
...
- A `QuestionRegistry` that stores question metadata, template type, rubric, evidence URI, creator, moderation status, and optional content linkage.
- A `RewardPoolEscrow` contract that holds allowlisted stablecoin funds per question and tracks question-specific deposits, refunds, tranches, and protocol fees.
- A `RewardPoolRewardDistributor` that offers pull-based stablecoin participation claims keyed by `contentId`, `roundId`, and token, using terminal round state and revealed-vote data but not paying based on winning side or unrelated question activity.
- Ponder/indexer tables such as `reward_pool_question`, `reward_pool_deposit`, `reward_pool_round_snapshot`, `reward_pool_claim`, and possibly `reward_pool_token`.
- Frontend flows for question creation, funding, voting, reveal, claim, cancellation, and dispute.
- Keeper support for settlement and possibly reward pool tranche release.
- Tests for solvency, refund paths, partial payout, stablecoin decimals, non-standard ERC20 behavior, and moderation removal.

The strongest architecture recommendation is to keep `RoundVotingEngine` unchanged for v1 reward pools. Curyo's settlement path is already complex and bytecode-sensitive, and stablecoin custody should not be added to the voting engine. Keep cREP voting as the judgment layer and add a separate escrow/distributor that reads settled outcomes.
...
- The Next submit flow assumes an approved platform URL and category. A reward-pool question flow should likely be a separate tab or route.
- Free transaction sponsorship should not cover stablecoin approvals or reward pool deposits in v1. Reward Pool funding should be self-funded until abuse and compliance economics are clearer.

High-risk test areas:

- Reward Pool escrow solvency.
...
- Unused question-reward pool rollover/refund behavior.
...
- Launch as "reward-pool review questions" or "verification reward pools."
...
- Do not use hard reward pool caps at launch; use recommended parameters, eligibility checks, monitoring, and emergency pause controls for larger pools.
- Allow only one allowlisted stablecoin at launch: USDC on Celo.
- Use pull-based claims.
- Gate reward pool funding and claims with Self.xyz OFAC and excluded-country checks, ideally through a reward pool-specific eligibility config.
...
- Add emergency controls to pause question creation, reward pool funding, media previewing, and claims separately.

Recommended v1 copy constraints:

- Use "review," "question," "confidence," "claim," "reward pool," and "reward."
...
Keep existing contracts unchanged. Define the funded-question model in docs and product requirements. Decide allowed templates, disallowed categories, payout split, invalid outcome, refund rules, and moderation policy.

### Phase 1: Question Metadata Without Stablecoin Claims

Create reward-pool questions with question-specific escrowed funds, but keep cREP voting unchanged and do not yet enable stablecoin reward claims. Show reward pool metadata in Ponder and the feed. Allow creator refund under clearly defined cancellation rules.

### Phase 2: Settled-Round Pull Claims

Enable pull-based, participation-only reward pool claims after terminal round resolution, paid only from the funded question's pool. Start with one allowlisted 6-decimal stablecoin, no hard reward pool caps, and stronger monitoring for larger pools. No stablecoin coherence bonus, no sponsored reward pool actions, no arbitrary tokens, and no media uploads.
...
Evaluate MACI-like receipt-freeness for high-value reward pools, category-specific reviewer reputation, expert queues, and broader media support.
