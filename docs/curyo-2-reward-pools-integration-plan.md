# Curyo 2.0 Question Reward Pools Integration Plan
...
This plan turns the Curyo 2.0 reward pool research into an implementation roadmap. It assumes the next Curyo version will redeploy contracts instead of upgrading the current deployment in place. That lets the protocol clean up the content model for reward-pool questions while reusing as much of the current frontend, cREP voting, commit-reveal, indexing, and documentation structure as possible.

The target product is not a prediction market. Curyo 2.0 should pay verified humans for question-scoped review participation. Stablecoin-funded reward pools should not create tradable yes/no positions, outcome-weighted stablecoin rewards, odds, or a winner-takes-pot flow.
...
4. Reuse and refactor frontend submit, voting, reward pool funding, and reward claiming flows.
5. Update docs, READMEs, whitepaper content, tests, and deployment playbooks.
6. Launch on testnet with monitoring, pause controls, and no hard reward pool caps.
...
- Question Reward Pool: optional USD-denominated Celo USDC funding attached during submission.
...
- Use `$` labels in high-level product surfaces: `$125 reward pool`, `Add $50`, `Claim $3.25`.
...
- Add an optional "Add reward pool" step or panel after the question details are valid.

The optional reward pool panel should support:
...
Question creation and reward pool funding can be two transactions at launch:

1. Submit the question.
2. Approve and fund the reward pool.

That avoids making the question submit transaction depend on ERC20 allowance state. The UI can still make it feel like one flow by taking the user directly into the reward pool funding step after the question is created.
...
- Reuse claim and reward surfaces, but add a stablecoin-funded question reward type distinct from cREP claims.
...
- Update API and TypeScript models so feed items expose question text, link, description, category/frame, and reward pool summary.
...
- Keep reward pool badges, category, rating, round status, and vote actions visible without making text-only cards feel broken or sparse.
- Make text-only cards layout-stable on mobile and dense laptop feed surfaces.

## Vote Card Changes

Voting surfaces should make reward pool funding visible without crowding the dense voting UI.

Recommended changes:

- Show a compact reward pool badge on each voting card, for example "$125 reward pool".
...
- In the existing "more" or details surface, add an "Add reward pool" action.
- The "Add reward pool" action should open a modal with USD amount, required voters, and required settled rounds.
- The modal should hide token selection at launch and state that reward pools are funded and paid in USDC on Celo.
...
The feed should still rank content by the protocol's normal discovery logic, but reward pool size can become an additional ranking or filter signal once abuse controls are in place.

## Reward UX

Stablecoin-funded question rewards should be shown separately from cREP rewards.

Recommended changes:

- Add claimable question reward rows to the rewards or profile claim area.
...
- `QuestionRewardPoolEscrow`: new stablecoin custody contract for question-scoped reward pool deposits, top-ups, token allowlist config, refund rules, and pause controls.
- `QuestionRewardPoolDistributor`: optional separate pull-claim contract. This can be folded into `QuestionRewardPoolEscrow` if bytecode size is manageable, but keeping it separate mirrors the current `RoundRewardDistributor` pattern.
- `ProtocolConfig`: add reward pool contract addresses only where shared address lookup is useful. Avoid making the voting engine responsible for stablecoin custody.
...
## Reward Pool Contract Model

Each reward pool should be scoped to one question. Top-ups with different terms should create a new reward pool rather than mutating the economic rules of an existing reward pool.

Recommended reward pool fields:
...
- `startRoundId`: first round eligible for this reward pool.
- `expiry`: deadline after which unmet reward pools can be refunded.
...
- `createRewardPool(uint256 questionId, address token, uint256 amount, uint32 requiredVoters, uint32 requiredSettledRounds, uint64 expiry)`.
- `addRewardPool(uint256 questionId, address token, uint256 amount, uint32 requiredVoters, uint32 requiredSettledRounds, uint64 expiry)`.
- `claimQuestionReward(uint256 rewardPoolId, uint256 roundId)`.
- `refundExpiredRewardPool(uint256 rewardPoolId)`.
...
If batched claims are added, they should enforce a small maximum batch length. Do not add an unbounded loop over rounds, voters, or reward pools.
...
- `RewardPoolRewardClaimed`.
- `RewardPoolRefunded`.
- `RewardPoolTokenAllowed`.
- `RewardPoolFundingPaused`.
- `RewardPoolClaimsPaused`.

## Reward Pool Payout Rules

The launch payout model should be deliberately simple:

1. A reward pool starts at the next round after funding is finalized. It should not apply retroactively to a round that already has commits.
2. A round qualifies only if it reaches the normal cREP settlement path, has at least `requiredVoters` revealed votes, and is at or after the reward pool start round.
...
- Do not auto-roll reward pools into future questions or the global Participation Reward Pool.
...
Curyo 2.0 should launch with Celo USDC as the only stablecoin-funded reward pool asset. USDT should not be supported initially.

The product should still feel USD-native in normal use:

- Feed, vote cards, submit forms, and reward pool modals should show primary amounts as `$125 reward pool`, `Add $50`, or `Claim $3.25`.
...
- Use the Celo USDC ERC20 token addresses for deposits and claims. Do not use Celo fee-currency adapter addresses for reward pool custody.
- Use OpenZeppelin `SafeERC20` for all token transfers.
- Store all reward pool accounting in the token's smallest unit.
...
Stablecoin funding and claims should require a reward pool-specific eligibility gate.

Recommended controls:

- Require Voter ID for voting, and require reward pool claim eligibility at claim time.
- Version the Self.xyz eligibility configuration so reward pool participation can require a newer OFAC and excluded-country policy than older Voter IDs.
- Apply stablecoin eligibility checks to reward pool funding and reward pool claims.
...
- Do not enforce hard reward pool caps at launch. Use eligibility checks, public reward pool terms, monitoring, and emergency pause controls instead.
- Preserve transaction records needed for user tax reporting: token, amount, date, question, and claim transaction.
- Resolve reward pool claims through the underlying Voter ID holder so delegated wallets remain safe without enabling duplicate claims across wallets.
...
### Reward Pool Eligibility Snapshot

The reward pool should not apply to a round that already has commits. If a reward pool can be attached to the current round, funders and voters can disagree about whether earlier voters were eligible, and funders can selectively fund after observing round activity. The contract should set `startRoundId` to the next round after the reward pool deposit is finalized.
...
- Use `claimed[rewardPoolId][roundId][voterId]` or equivalent.
- Reject claims if the caller cannot prove current authority for the snapshotted Voter ID.
- Bind any off-chain payout authorization to chain ID, escrow contract, reward pool ID, round ID, Voter ID, payout address, nonce, and deadline.
...
A question submitter, reward pool funder, and Voter IDs associated with them should not be eligible to claim that question's reward pool unless governance deliberately enables self-review for a category. Otherwise a funder can recycle their own reward pool through controlled Voter IDs and distort feed incentives or metrics.

The launch rule should be:

- Submitter Voter ID cannot claim its own question reward pool.
- Funder Voter ID cannot claim the reward pool it funded.
- If multiple funders add separate reward pools to the same question, each funder is excluded only from the reward pool they funded unless a stricter category policy applies.
...
### No Hard Reward Pool Caps

No hard reward pool caps is a product decision, but it increases security and abuse blast radius. The launch plan should compensate without silently reintroducing a cap:

- Show stronger warnings and recommended parameters for larger reward pools.
- Monitor large reward pool creation, rapid repeated funding, and funder/voter clustering.
...
- Treat large-reward pool categories such as trust and safety, public allegations, or financial claims as moderation-sensitive.
...
- Reward Pool exists and is funded.
- Round qualifies.
...
Ponder should index reward pools directly rather than deriving them ad hoc in the frontend.
...
- `reward_pool_refund`: funder refunds and cancelled/expired reward pool recovery.
- `reward_pool_token`: allowlisted token metadata by chain.

Recommended API additions:

- Add active reward pool summaries to content/feed items.
...
- Add reward pool terms for the More section and modal review screen.
- Add user-specific claimable question rewards to existing reward aggregation.
- Add refundability and expiry state for funder views.

Suggested feed item fields:

```ts
type RewardPoolSummary = {
  id: string;
  token: `0x${string}`;
  symbol: "USDC";
  usdLabel: string;
...
Question feed responses should include `activeRewardPools`, a formatted `totalRewardPoolLabel` such as `$125`, media render metadata, and user-specific `claimableQuestionRewards` where available.
...
- Deploy `QuestionRewardPoolEscrow` and optional `QuestionRewardPoolDistributor`.
- Wire the voting engine, registry, reward pool escrow, reward distributor, participation pool, and protocol config.
...
- Export ABIs for the new reward pool contracts.
...
Stablecoin approval and reward pool funding should not be sponsored at launch. The existing sponsored/free transaction logic should remain conservative and can continue covering known cREP and registry flows only.
...
2. `contracts: add question reward pool escrow`
   ...
   - Exclude submitter and funder Voter IDs from claiming their own reward pool unless a future category policy explicitly allows it.
...
4. `ponder: index question reward pools`
   - Add reward pool schema tables, event handlers, contract config, tests, and API fields.
   - Include active reward pool summaries and user claim state in feed/reward responses.
...
6. `nextjs: add reward pool funding UI`
   - Add optional reward pool funding after submission.
   - Add USD amount input, required voter input, required settled round input, and USDC approve/deposit flow.
   - Default new reward pools to 5 required voters and 2 settled rounds, while allowing custom values down to 3 voters and 1 settled round.

7. `nextjs: show and fund reward pools from vote cards`
   - Add compact reward pool badges to feed/vote cards.
   ...
   - Add "Add reward pool" in the More section and open the funding modal from there.
...
9. `sdk/bot/keeper: expose reward pool helpers`
   - Add SDK reads for questions, reward pool summaries, and claimable question rewards.
   - Add bot support only if automated reward pool monitoring is needed.
   - Add keeper support only if reward pool qualification snapshots, retries, or repair jobs become necessary.
...
- Index `RewardPoolCreated`, `RewardPoolFunded`, `RewardPoolRoundQualified`, `RewardPoolRewardClaimed`, `RewardPoolRefunded`, and token allowlist events.
- Rebuild active reward pool aggregate after reorg-safe event replay.
- Return USDC amounts and USD labels in feed responses.
- Return user claimable question rewards.

Next.js tests:

- Submit question without a reward pool.
- Submit text-only subjective question.
- Submit question with image link.
- Submit question with YouTube link.
- Submit question and continue into optional reward pool funding.
...
- Render reward pool badges on vote cards without layout shift.
...
- Open the Add Reward Pool modal from the More section.
- Handle USDC allowance and deposit.
- Default reward pool form to 5 required voters and 2 settled rounds.
- Allow custom reward pool values down to 3 voters and 1 settled round.
- Confirm no USDT or generic token selector appears in the launch funding UI.
- Show stablecoin-funded question reward claims separately from cREP.

End-to-end tests:

- Submit question, fund question reward, vote, reveal, settle, and claim stablecoin reward.
...
- Verify submitter and funder Voter IDs cannot claim excluded reward pools.
- Verify a reward pool funded mid-round only starts with the following round.
...
- Check mobile and laptop layouts for reward pool badges, More modal, and claim rows.
...
- `README.md`: explain Curyo 2.0 as reward-pool questions and summarize the new package responsibilities.
- `docs/curyo-2-reward-pools-research.md`: keep the research memo aligned with final decisions if the plan changes.
- `docs/curyo-2-reward-pools-integration-plan.md`: update as implementation decisions land.
- `packages/foundry/README.md`: document `QuestionRegistry`, reward pool escrow/distributor, Celo USDC allowlist, deployment config, and test commands.
- `packages/ponder/README.md`: document reward pool tables, handlers, API fields, and env vars.
- `packages/nextjs/README.md`: document text-only questions, image and YouTube link previews, submit, reward pool funding, reward claim, and whitepaper generation flows.
- `packages/sdk/README.md`: document question and reward pool helper APIs.
- `packages/bot/README.md`: update only if bots can submit, monitor, or claim reward pools.
- `packages/keeper/README.md`: update only if keeper gets reward pool qualification or repair duties.
...
5. Deploy to Celo Sepolia with default categories seeded and no hard reward pool caps.
6. Run a controlled testnet pilot with known voters and small reward pools.
7. Review moderation, compliance, claim, refund, and layout issues.
8. Freeze contract interfaces for audit.
9. Deploy to Celo mainnet with allowlisted Celo USDC only and no hard reward pool caps.
10. Keep monitoring reward pool sizes, issuer risk, and moderation workload after launch.
...
- Funding and claiming require reward pool eligibility checks.
- Emergency pause for new reward pool funding.
