...
- Owner rotation can be disabled or heavily restricted in a 4337 design. Doing that makes the smart-account identity model closer to today's EOA model, but it also gives up some of the recovery and flexibility that make smart accounts attractive in the first place.
- This is still not the same as today's delegation model in [VoterIdNFT.sol](/Users/davidhawig/source/curyo-release/packages/foundry/contracts/VoterIdNFT.sol). A delegate can pass some identity-gated checks such as voting and submission flows, but contracts like [ProfileRegistry.sol](/Users/davidhawig/source/curyo-release/packages/foundry/contracts/ProfileRegistry.sol), [FrontendRegistry.sol](/Users/davidhawig/source/curyo-release/packages/foundry/contracts/FrontendRegistry.sol), and [CategoryRegistry.sol](/Users/davidhawig/source/curyo-release/packages/foundry/contracts/CategoryRegistry.sol) explicitly require the caller to be the holder, not just a delegate.
- A rotated smart-account controller would normally inherit the full holder position of that account, not just delegated permissions. That is a materially broader power than the current delegate model unless Curyo intentionally builds a much more restricted smart-account policy.
