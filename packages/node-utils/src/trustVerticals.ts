...
export const TRUST_VERTICALS = [
  ...
  },
  {
    ...
    label: "Investment",
    description:
      "Public market, crypto, and investment-promotion claim credibility.",
  ...
  },
  {
    ...
    label: "News",
    description:
      "Current events, public claims, civic information, and source trust.",
  ...
  },
  {
    ...
    label: "Software",
    description:
      "Developer tools, repositories, packages, models, and supply-chain trust.",
  ...
  },
  {
    ...
    label: "Entertainment",
    description:
      "Media, games, books, streams, cards, podcasts, and cultural content.",
...
const TRUST_VERTICAL_SLUGS = new Set<string>(
  TRUST_VERTICALS.map((vertical) => vertical.slug),
);
...
function normalizeCategoryId(
  categoryId: ResolveTrustVerticalInput["categoryId"],
) {
...
function normalizeDomain(domain: string) {
  ...
  const host =
    withoutProtocol.split(/[/?#:]/, 1)[0]?.replace(/^www\./, "") ?? "";
...
export function getTrustVertical(slug: TrustVerticalSlug): TrustVertical {
  return (
    TRUST_VERTICAL_BY_SLUG.get(slug) ??
    TRUST_VERTICAL_BY_SLUG.get(DEFAULT_TRUST_VERTICAL_SLUG)!
  );
}
...
export function parseTagsValue(
...
export function extractTrustVerticalFromTags(
  tags: readonly string[] | string | null | undefined,
) {
...
export function stripTrustVerticalTags(
  tags: readonly string[] | string | null | undefined,
): string[] {
  return parseTagsValue(tags).filter((tag) => !isReservedTrustVerticalTag(tag));
}

export function mergeTrustVerticalTag(
  tags: readonly string[] | string | null | undefined,
  slug: TrustVerticalSlug,
) {
  return [...stripTrustVerticalTags(tags), buildTrustVerticalTag(slug)];
}

export function resolveTrustVerticalSlug(
  input: ResolveTrustVerticalInput,
): TrustVerticalSlug {
  ...
  const categoryName = input.categoryName
    ? normalizeLookupValue(input.categoryName)
    : "";
  ...
  const domain = input.domain
    ...
    : input.url
      ? extractDomainFromUrl(input.url)
      : "";
...
export function getLegacyCategoryIdsForTrustVertical(
  slug: TrustVerticalSlug,
): readonly string[] {
  return LEGACY_CATEGORY_IDS_BY_VERTICAL[slug] ?? [];
}

export function getLegacyDomainsForTrustVertical(
  slug: TrustVerticalSlug,
): readonly string[] {
