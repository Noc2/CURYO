...
export interface CuryoAgentClient {
  ...
  getQuestionStatus(params: QuestionStatusLookup): Promise<QuestionStatusResponse>;
  getResult(params: QuestionStatusLookup & { contentId?: string | bigint }): Promise<CuryoAgentResult>;
...
export function createCuryoAgentClient(options: CuryoAgentClientOptions = {}): CuryoAgentClient {
...
export function quoteQuestion(
  ...
  return callMcpTool<QuoteQuestionResponse>(normalizeAgentConfig(options), "curyo_quote_question", params);
}
...
export async function getQuestionStatus(
  ...
  if (config.mcpAccessToken) {
    return callMcpTool<QuestionStatusResponse>(config, "curyo_get_question_status", { ...params });
  }

  return requestJson<QuestionStatusResponse>(config, x402StatusUrl(config, params), {
    headers: {
      accept: "application/json",
    },
    method: "GET",
...
export async function getResult(
  ...
  const result = await callMcpTool<unknown>(normalizeAgentConfig(options), "curyo_get_result", {
    ...params,
    contentId: params.contentId === undefined ? undefined : String(params.contentId),
  });
...
export async function listResultTemplates(
  ...
  return callMcpTool<ListResultTemplatesResponse>(normalizeAgentConfig(options), "curyo_list_result_templates", {});
}
...
export function parseAgentResult(value: unknown): CuryoAgentResult {
  ...
  const ready = typeof unwrapped.ready === "boolean" ? unwrapped.ready : inferResultReady(unwrapped);
  return {
    ...unwrapped,
    ready,
  } as CuryoAgentResult;
...
export function buildWebhookVerifier(options: WebhookVerifierOptions): WebhookVerifier {
  ...
  const eventIdHeader = (options.eventIdHeader ?? "x-curyo-callback-id").toLowerCase();
  const signatureHeader = (options.signatureHeader ?? "x-curyo-callback-signature").toLowerCase();
  const timestampHeader = (options.timestampHeader ?? "x-curyo-callback-timestamp").toLowerCase();
  ...
  async function verify(params: VerifyWebhookParams): Promise<boolean> {
    ...
    if (!timestamp || (toleranceSeconds >= 0 && !isTimestampFresh(timestamp, toleranceSeconds, params.now))) {
      return false;
    }
...
async function callMcpTool<T>(config: NormalizedAgentConfig, name: string, args: JsonRecord): Promise<T> {
  if (!config.mcpApiUrl) {
    throw new CuryoSdkError("apiBaseUrl or mcpApiUrl is required for MCP agent operations");
  }
  ...
  if (isJsonRecord(rpc.error)) {
    const message = typeof rpc.error.message === "string" ? rpc.error.message : "Curyo MCP request failed";
    throw new CuryoApiError(message, 400);
  }

  const result = isJsonRecord(rpc.result) ? rpc.result : null;
  const toolResult = isJsonRecord(result?.structuredContent) ? result.structuredContent : result?.structuredContent;
  if (isJsonRecord(toolResult) && toolResult.isError === true) {
    const message = typeof toolResult.message === "string" ? toolResult.message : "Curyo MCP tool failed";
    throw new CuryoApiError(message, 400);
  }
  if (result?.isError === true) {
    const structured = isJsonRecord(result.structuredContent) ? result.structuredContent : {};
    const message = typeof structured.message === "string" ? structured.message : "Curyo MCP tool failed";
    throw new CuryoApiError(message, 400);
  }

  return (result && "structuredContent" in result ? result.structuredContent : rpc.result) as T;
}
...
function normalizeAgentConfig(options: CuryoAgentClientOptions): NormalizedAgentConfig {
  const apiBaseUrl = normalizeUrl(options.apiBaseUrl);
  return {
    apiBaseUrl,
    fetchImpl: options.fetchImpl ?? fetch,
    mcpAccessToken: options.mcpAccessToken,
    mcpApiUrl: normalizeUrl(options.mcpApiUrl) ?? (apiBaseUrl ? new URL(DEFAULT_MCP_PATH, `${apiBaseUrl}/`).toString() : undefined),
    mcpProtocolVersion: options.mcpProtocolVersion ?? DEFAULT_MCP_PROTOCOL_VERSION,
...
function x402QuestionsUrl(config: NormalizedAgentConfig) {
  if (!config.apiBaseUrl) {
    throw new CuryoSdkError("apiBaseUrl is required for x402 askHumans operations");
  }

  return new URL(config.x402QuestionsPath, `${config.apiBaseUrl}/`).toString();
}

function x402StatusUrl(config: NormalizedAgentConfig, params: QuestionStatusLookup) {
  if (!config.apiBaseUrl) {
    throw new CuryoSdkError("apiBaseUrl is required for x402 status operations");
  }
  const operationKey = typeof params.operationKey === "string" ? params.operationKey.trim() : "";

  if (operationKey) {
    return new URL(`${config.x402QuestionsPath.replace(/\/+$/, "")}/${operationKey}`, `${config.apiBaseUrl}/`).toString();
  }

  if (!params.chainId || !params.clientRequestId) {
    throw new CuryoSdkError("Provide operationKey or both chainId and clientRequestId");
  }

  const url = new URL(`${config.x402QuestionsPath.replace(/\/+$/, "")}/by-client-request`, `${config.apiBaseUrl}/`);
...
function unwrapStructuredContent(value: unknown): unknown {
  ...
  if (isJsonRecord(value.result)) {
    if (isJsonRecord(value.result.structuredContent)) return value.result.structuredContent;
    return value.result;
  }
  if (Array.isArray(value.content)) {
    const textPart = value.content.find(part => isJsonRecord(part) && part.type === "text" && typeof part.text === "string");
...
function inferResultReady(value: JsonRecord): boolean {
  if (isJsonRecord(value.result) && typeof value.result.ready === "boolean") return value.result.ready;
  if (value.result === null) return false;
  if (typeof value.status === "string") return value.status === "settled" || value.status === "submitted";
  return false;
}
...
function getHeader(headers: Headers | Record<string, string | string[] | undefined | null>, name: string) {
...
function isTimestampFresh(timestamp: string, toleranceSeconds: number, now: Date | number | undefined) {
