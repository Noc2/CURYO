...
function StatCard({ label, value, detail }: { label: string; value: string; detail?: string }) {
  return (
    <div className="rounded-2xl bg-base-content/[0.05] px-4 py-3">
      <div className="text-base text-base-content/45">{label}</div>
      <div className="mt-1 text-2xl font-semibold tabular-nums">{value}</div>
      {detail ? <div className="mt-1 text-base text-base-content/45">{detail}</div> : null}
    </div>
  );
}
...
export function PublicProfileView({ address }: PublicProfileViewProps) {
  ...
  const hasProfileRecord = totalVotes > 0 || totalContent > 0 || !!summary?.name || !!summary?.imageUrl;
      ...
      <div className="w-full max-w-5xl space-y-6">
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-base text-base-content/45">Public profile</div>
                <h1 className="mt-1 truncate text-3xl font-semibold">{displayName}</h1>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            detail={ownProfile ? "Your current balance" : "Current token balance"}
          />
          <StatCard
            label="Resolved votes"
            value={stats ? String(stats.totalSettledVotes) : "0"}
            detail={stats ? `${stats.totalWins}W / ${stats.totalLosses}L` : "No settled rounds yet"}
          />
          <StatCard
            label="Claimed rewards"
            value={`${formatCrepString(totalRewardsClaimed)} cREP`}
            detail={totalVotes > 0 ? `${totalVotes} total votes recorded` : "No claims recorded yet"}
          />
          <StatCard
            label="Best streak"
            value={stats ? `${stats.bestWinStreak}W` : "0"}
            detail={stats ? `Current streak ${streakLabel}` : "Waiting for first streak"}
          />
        </div>
        ...
        <div className="surface-card rounded-3xl p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <span className="text-base tabular-nums text-base-content/60">
              {stats ? `${stats.totalSettledVotes} resolved votes` : "No resolved votes yet"}
            </span>
          </div>
        ...
        <div className="surface-card rounded-3xl p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <span className="text-base text-base-content/45">
              {votesLoading ? "Loading activity..." : `${recentVotes.length} most recent votes`}
            </span>
          </div>
