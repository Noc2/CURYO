...
import { getVoteFeedThumbnailSrc } from "~~/components/vote/VoteFeedCards";

const ALL_FILTER = "All";
export const BROKEN_FILTER = "Broken";
const slugify = (name: string) => name.toLowerCase().replace(/\s+/g, "-");
type SortOption = "for_you" | "newest" | "oldest" | "highest_rated" | "lowest_rated";
export type SearchSortOption = Exclude<SortOption, "for_you">;
...
export function useVotePageController() {
  ...
  const votedContentOrder = useMemo(() => {
    ...
    for (const vote of votes) {
      const voteContentId = vote.contentId.toString();
      if (seen.has(voteContentId)) continue;
      seen.add(voteContentId);
  ...
  const voteOrderMap = useMemo(() => {
    ...
    votes.forEach((vote, index) => {
      const voteContentId = vote.contentId.toString();
      if (!order.has(voteContentId)) {
        order.set(voteContentId, index);
      }
    });
    return order;
  }, [votes]);
  const settlingSoonOrderMap = useMemo(() => {
    ...
    discoverSignals.settlingSoon.forEach((item, index) => {
      const itemContentId = item.contentId.toString();
      if (!order.has(itemContentId)) {
        order.set(itemContentId, index);
      }
    });
    return order;
  }, [discoverSignals.settlingSoon]);
  ...
  const submitterAddresses = useMemo(() => visibleFeedItems.map(item => item.submitter), [visibleFeedItems]);
  ...
  const enrichedProfiles = useMemo(() => {
    ...
    for (const [submitterAddress, profile] of Object.entries(submitterProfiles)) {
      const stats = accuracyMap[submitterAddress];
      result[submitterAddress] = {
        ...profile,
  ...
  const handleConfirmStake = useCallback(
    async (stakeAmount: number) => {
      if (stakeModalCooldownSeconds > 0) {
        notification.info(getVoteCooldownMessage(stakeModalCooldownSeconds), { duration: 6000 });
        setStakeModal(prev => ({ ...prev, isOpen: false }));
        return;
      }

      const item = displayFeed.find(feedItem => feedItem.id === stakeModal.contentId);
  ...
  const categories = useMemo(() => {
    ...
    const nextCategories = [ALL_FILTER, ...sorted.map(cat => cat.name)];
    if (brokenCount > 0) nextCategories.push(BROKEN_FILTER);
  ...
  useEffect(() => {
    const applyHash = () => {
      ...
      if (!hash) return;
      const match = categories.find(categoryName => slugify(categoryName) === hash);
  ...
  return {
    ...
    stakeModal,
    stakeModalCooldownSeconds,
    supportedTouchNavigation: supportsTouchNavigation,
    view,
    viewGroups,
    visibleFeedItems,
    voteError,
