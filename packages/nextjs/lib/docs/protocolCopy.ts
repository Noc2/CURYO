...
export const protocolCopy = {
  ...
  governanceOverview:
    "Curyo is designed to finalize into a community-governed system with split control domains. The governor/timelock owns upgrade and config powers, while treasury routing is assigned to a separate treasury authority so one compromise does not automatically combine code upgrades and treasury control. Local or pre-finalization environments may still use temporary deployer wiring during setup.",
  governanceDesignPrinciple:
    "Finalized deployments keep upgrade authority and treasury authority separate, and temporary deployer setup roles are renounced after deployment finalization. Local or pre-finalization environments may still use temporary deployer wiring during setup.",
} as const;
