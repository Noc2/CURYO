...
export const protocolCopy = {
  ...
  strongerSignalOverview:
    "Curyo offers a stronger signal: question-first submissions, optional Question Reward Pools, and stake-weighted ratings from verified humans, recorded publicly and backed by economic risk.",
  predictionGamesOverview: `Curyo replaces passive likes with question-first rounds and optional Question Reward Pools. Submissions start as a question and can be text only or include a regular evidence link, direct image link, or YouTube link. Reward pools are paid in USDC on Celo and displayed to users as USD, while voters predict whether the content's rating will go up or down and back those predictions with cREP token stakes. The majority side wins the content-specific voter pool: revealed losers can reclaim ${protocolDocFacts.revealedLoserRefundPercentLabel} of raw stake, and the remaining losing pool is split across winners, submitters, platform fees, consensus reserve, and treasury according to fixed on-chain percentages.`,
  contributorRewardsOverview: `After a ${protocolDocFacts.revealedLoserRefundPercentLabel} rebate for revealed losers, the remaining losing stake funds the content-specific voter pool plus submitter, category, frontend, consensus, and treasury shares. Question Reward Pools add a separate USDC-on-Celo reward path for eligible revealed voters, with a 3% commit-attributed frontend-operator share inside each qualified reward-pool round.`,
  participationPoolPurpose:
    "Bootstraps early adoption -- voter bootstrap rewards become claimable after round settlement, and healthy submitter bootstrap rewards are snapshotted when submitter stakes return (rate halving schedule).",
  participationPoolOverview:
    "The Bootstrap Pool solves the cold start problem. When the platform is new and vote stakes are small, round rewards alone may not be enough to attract voters and submitters. The Bootstrap Pool pays proportional cREP bonuses based on stake amount: winning revealed voters claim bootstrap rewards after round settlement, while submitter bootstrap rewards are only snapshotted when the submitter stake resolves on the healthy path after a settled round. The voter reward rate is snapshotted at resolution time for fairness. Early participants receive the most thanks to a halving schedule as cumulative rewards grow and the reward rate decreases.",
  governanceOverview:
