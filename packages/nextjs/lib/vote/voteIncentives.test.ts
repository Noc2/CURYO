...
test("estimateVoteReturn uses informed weight during open phase", () => {
  ...
  assert.equal(estimate.projectedVoterPoolMicro, 31_160_000n);
  assert.equal(estimate.projectedPoolShareMicro, 3_462_222n);
  assert.equal(estimate.estimatedGrossReturnMicro, 13_462_222n);
...
test("estimateVoteReturn keeps full weight during blind phase", () => {
  ...
  assert.equal(estimate.projectedPoolShareMicro, 10_386_666n);
  assert.equal(estimate.estimatedGrossReturnMicro, 20_386_666n);
});
