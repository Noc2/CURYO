...
- OpenClaw-style clients should use the same OpenClaw-ready flow documented in the AI docs: configure an agent token, list templates, quote, ask, wait for a callback or status read, then fetch the structured result. Operator-facing token lifecycle, scopes, daily budgets, per-ask caps, category allowlists, pauses, and audit history belong in `/settings?tab=agents`; until that UI fully replaces static config, `CURYO_MCP_AGENTS` remains the source of truth for managed MCP agent registration.
