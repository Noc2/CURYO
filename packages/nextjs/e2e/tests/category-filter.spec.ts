...
test.describe("Trust vertical filter", () => {
  ...
  /**
   * Helper: find the first visible trust vertical pill that isn't "All".
   * Waits for the "All" button to appear, then finds sibling vertical buttons.
   * Returns { name, locator } or null if none found.
   */
  async function findVisibleVerticalPill(page: any) {
    ...
    // Prefer verticals that the local deploy helper seeds with content so the
    // assertions don't pick an empty vertical and stall on a blank feed.
    const knownVerticals = ["Entertainment", "Investment", "Software", "People", "Products", "Health", "News"];
  ...
  test("clicking vertical pill updates URL hash and filters feed", async ({ connectedPage: page }) => {
    await loadVoteFeed(page);

    // Find any visible vertical pill
    const pill = await findVisibleVerticalPill(page);
    ...
    // The filter shell should remain usable after the hash change even when
    // the selected vertical has no immediately rendered feed rows.
  ...
  test("clicking All clears URL hash", async ({ connectedPage: page }) => {
    // Start with a vertical hash
    await loadVoteFeed(page, "/vote#entertainment");
  ...
  test("URL hash on load activates corresponding vertical", async ({ connectedPage: page }) => {
    // Find a visible vertical on the default page first
    await loadVoteFeed(page);
  ...
  test("overflow dropdown opens with search", async ({ connectedPage: page }) => {
    ...
    const searchInput = page.getByRole("textbox", { name: "Search trust verticals" });
  ...
  test("vertical filter shows content or empty state", async ({ connectedPage: page }) => {
    await loadVoteFeed(page);
    ...
    // Wait for feed to re-render after vertical change
    await waitForFeedLoaded(page, 5_000);
