...
- `submit_content`: reserve and reveal a question-first submission through `ContentRegistry.submitQuestion`, with text-only, link, image, or YouTube-link inputs and optional reward pools displayed as USD even though settlement is paid in USDC on Celo with the default eligible frontend-operator share
