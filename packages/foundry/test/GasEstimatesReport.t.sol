...
contract UserTransactionGasEstimatesTest is RoundIntegrationTest {
    ...
    function testGasEstimate_approveForSubmit_logs() public {
        ...
        uint256 gasUsed = _measureCallAs(
            submitter, address(crepToken), abi.encodeCall(IERC20.approve, (address(registry), 10e6))
        );
    ...
    function testGasEstimate_submitContent_logs() public {
        ...
        uint256 gasUsed = _measureCallAs(
            submitter,
            address(registry),
            abi.encodeCall(ContentRegistry.submitContent, ("https://example.com/gas-report", "test goal", "test goal", "test", 0))
        );
    ...
    function testGasEstimate_voteTransferAndCall_logs() public {
        ...
        bytes memory payload = abi.encode(contentId, commitHash, ciphertext, address(0));

        uint256 gasUsed = _measureCallAs(
            voter1,
            address(crepToken),
            abi.encodeWithSignature(
                "transferAndCall(address,uint256,bytes)", address(votingEngine), STAKE, payload
            )
        );
    ...
    function testGasEstimate_voteTransferAndCallWithApprovedFrontend_logs() public {
        ...
        bytes32 commitHash = _commitHash(true, salt, contentId);
        bytes memory ciphertext = _testCiphertext(true, salt, contentId);
        bytes memory payload = abi.encode(contentId, commitHash, ciphertext, frontendOp);
...
contract FrontendTransactionGasEstimatesTest is Test {
    ...
    function testGasEstimate_frontendApproveStake_logs() public {
        vm.pauseGasMetering();
        uint256 gasUsed = _measureCallAs(
            frontend, address(crepToken), abi.encodeCall(IERC20.approve, (address(registry), STAKE))
        );
...
contract IdentityTransactionGasEstimatesTest is Test {
    ...
    function testGasEstimate_setDelegate_logs() public {
        ...
        uint256 gasUsed =
            _measureCallAs(user1, address(voterIdNFT), abi.encodeCall(VoterIdNFT.setDelegate, (delegate)));
